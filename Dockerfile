...
RUN bash -c 'export URL=$(echo $(if [[ $current_version == "8.0.43" || $current_version == "8.4.6" || $current_version == "9.4.0" ]]; then echo "https://cdn.mysql.com/Downloads/MySQL-"; else echo "https://cdn.mysql.com/archives/mysql-"; fi)$(echo $major_minor_version)/mysql-$(echo $mysql_version).tar.gz)'
